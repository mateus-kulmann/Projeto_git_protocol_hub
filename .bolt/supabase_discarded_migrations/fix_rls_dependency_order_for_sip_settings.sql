/*
  # Correção da Ordem de Dependência das Políticas RLS para sip_settings

  Esta migração corrige o erro de dependência encontrado na migração anterior.
  O erro ocorria porque a função `get_current_user_role()` era removida antes
  das políticas que dependiam dela.

  A ordem correta é:
  1. Remover todas as políticas RLS da tabela `sip_settings`.
  2. Remover a função auxiliar `get_current_user_role()`.
  3. Criar as novas políticas com a lógica de verificação de papéis embutida.
*/

-- 1. Remover as políticas existentes PRIMEIRO para resolver a dependência
...
-- 2. AGORA, remover a função auxiliar que não tem mais dependências
