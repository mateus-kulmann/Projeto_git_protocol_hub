...
interface EditSolicitanteModalProps {
  isOpen: boolean;
  onClose: () => void;
...
interface EditSolicitanteFormData {
  tipo: 'pessoa_fisica' | 'pessoa_juridica';
  nome_completo?: string;
  razao_social?: string;
  cpf?: string;
...
const EditSolicitanteModal: React.FC<EditSolicitanteModalProps> = ({
  isOpen,
  onClose,
  ...
  const { register, handleSubmit, formState: { errors }, reset, watch } = useForm<EditSolicitanteFormData>({
    defaultValues: {
      tipo: solicitante.tipo,
      nome_completo: solicitante.nome_completo || '',
      razao_social: solicitante.razao_social || '',
      cpf: solicitante.cpf || '',
      cnpj: solicitante.cnpj || '',
      email: solicitante.email || '',
      telefone: solicitante.telefone || '',
      endereco: solicitante.endereco || '',
      pessoa_juridica_id: solicitante.pessoa_juridica_id || ''
    }
  });
  ...
  }, [isOpen, solicitante, reset]);

  const loadPessoasJuridicas = async () => {
    try {
      const data = await apiService.getSolicitantes({ tipo: 'pessoa_juridica' });
      // Filter out current solicitante if it's PJ to prevent self-reference
      setPessoasJuridicas(data.filter(pj => pj.id !== solicitante.id));
    } catch (error) {
  ...
  const onSubmit = async (data: EditSolicitanteFormData) => {
    try {
      setLoading(true);
      ...
      await apiService.updateSolicitante(solicitante.id, data);
      ...
      toast.error(error.message || 'Erro ao atualizar solicitante');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="border-b border-gray-200 p-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-bold text-[#404040]">Editar Solicitante</h2>
            </div>
            <button
              onClick={handleClose}
...
export default EditSolicitanteModal;