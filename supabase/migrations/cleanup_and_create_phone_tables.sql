/*
  # Cleanup and Create Phone System Tables
  
  1. Drop existing tables if they exist (in correct order)
...
DROP TABLE IF EXISTS sip_settings CASCADE;
...
-- Add circular reference from call_recordings to call_history
...
CREATE INDEX idx_sip_settings_user_id ON sip_settings(user_id);
CREATE INDEX idx_call_history_user_id ON call_history(user_id);
CREATE INDEX idx_call_history_protocol_id ON call_history(protocol_id);
CREATE INDEX idx_call_history_start_time ON call_history(start_time DESC);
CREATE INDEX idx_call_recordings_call_history_id ON call_recordings(call_history_id);
